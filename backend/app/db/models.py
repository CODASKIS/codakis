...
class Forfait(Base):
    ...
    duree_mois: Mapped[int] = mapped_column(Integer, default=1, nullable=False)
...
class SeancePratique(Base):
    ...
    participation: Mapped[str] = mapped_column(String(16), default="en_attente", nullable=False)
...
class SimulationScenario(Base):
    ...
    created_by: Mapped[uuid.UUID | None] = mapped_column(Uuid(as_uuid=True), ForeignKey("utilisateurs.id"), nullable=True)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now())
    updated_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now(), onupdate=func.now())
